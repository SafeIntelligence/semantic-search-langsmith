...
> **What changed?** This template now focuses solely on semantic search over MongoDB Atlas. It retrieves with vector search, reranks the results, and returns a ranked list of documents—no chat-model responses are generated.
...
The retrieval flow now only performs semantic search and reranking. It:

1. Takes a user **query** as input
2. Searches MongoDB Atlas for documents filtered by user_id
3. Reranks the retrieved documents and returns the ranked list

By default, it returns the ordered documents (no LLM-generated text), filtered by the user's ID for personalized results.

If you set `COHERE_API_KEY`, the reranker will use Cohere's `rerank-english-v3.0`; otherwise, it falls back to the scores returned by MongoDB vector search.
